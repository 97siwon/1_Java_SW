...
public class StudentManagementService {
	...
	public StudentManagementService() {
		...
		stdArr[1] = new Student(2, 3, 12, "박철수", 50, 80, 40);
		stdArr[2] = new Student(1, 6, 22, "김미영", 80, 50, 90);
		stdArr[3] = new Student(1, 6, 15, "장원영", 80, 40, 90);
	 ...
	 * @param number
	 * @param name
	 * @return 0(null 인덱스 없음) 또는 1(null 인덱스가 있어서 학생 갹체의 주소를 추가함.)
	 */
	public int addStudent(int grade, int classRoom, int number, String name) {
		...
		if(idx == -1) { // null인 인덱스가 없는 경우 / 빈자리가 없으면
			return 0; // 현재 메서드를 종료하고 호출한 곳으로 0을 가지고 돌아감.
			
		}
		
		
		// null인 인덱스에 학생 객체를 생성헤서 주소를 대입 / 4번 인덱스에 대입
